...
problems, particularly on Windows.  However, on Ubuntu, it is very easy to
...
tools, and \texttt{Rmpi} works well there, also.
...
\texttt{Rmpi} is to use multiple machines.  To do that, you'll need to
start your \texttt{R} session using a command such as \texttt{orterun},
...
deals with programming issues, and for running \texttt{doMPI} scripts,
I'll only discuss the use of \texttt{OpenMPI}.  Very likely you will
have some problem that I can't help you with, and so I highly recommend
that you join the R-sig-hpc mailing list.  It is a very friendly and
helpful community, and there isn't even a lot of traffic on it.
...
That's the most fool-proof way to install \texttt{Rmpi} on Ubuntu.  But
on other systems, including Mac~OS~X, you can install it with:
...
As you can see from the friendly message\footnote{The message is from
