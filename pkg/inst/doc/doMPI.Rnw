...
well with it.  Mac~OS~X~10.5 and later includes \texttt{Open MPI} with
...
have seen error messages from \texttt{orterun} when I forgot to
finalize.
...
\texttt{mpirun}, \texttt{mpiexec}, or in the case of \texttt{Open MPI},
\texttt{orterun}\footnote{There could be another way to run on multiple
computers, but I haven't figured it out.  But I had a hard time finding
documentation on this subject, especially in the context of
\texttt{Rmpi}.}.  Since I've only used \texttt{Open MPI}, I'll only
discuss the use of \texttt{orterun}, although it should be pretty simple
to translate my instructions for other implementations.

When you execute \texttt{orterun}, you can specify the hosts to use
...
you wanted using \texttt{orterun}\footnote{Actually,
...
\begin{verbatim}
% orterun -H localhost,n2,n3 R --slave -f sincMPI.R
\end{verbatim}

The \texttt{orterun} command will run the \texttt{R} interpreter on
...
\begin{verbatim}
% orterun -H n1,n2,n3 -n 1 R --slave -f sincMPI.R
\end{verbatim}

Because we specified \texttt{-n 1}, the \texttt{orterun} command only
...
\begin{verbatim}
% orterun -H localhost,n2,n3 -n 6 R --slave -f sincMPI.R
\end{verbatim}
...
\texttt{orterun} command, which is what we've been doing in all of these
