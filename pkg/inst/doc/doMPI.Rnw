...
so you can clearly see if it installs correctly or not.  On Ubuntu, you
can actually install \texttt{Rmpi} by using \texttt{apt-get} to install
the Debian \texttt{r-cran-rmpi} package.  That's the most fool-proof way
to install \texttt{Rmpi} on Ubuntu.  But on other systems, including
Mac~OS~X, you can install it with:
...
But don't be too surprised if someone\footnote{Like Dirk, for example.}
suggests that you switch to Ubuntu.
