...
the case of \texttt{OpenMPI}, \texttt{orterun}.  Since I have only ever
...
script that they execute is called \texttt{RMPIworker.R}, which is in
the \texttt{doMPI} installation directory.
...
can spawn workers on other computers.  This is the way that I do it:
...
use.  The \texttt{-n} option says to run the command on only one
...
That may seem a bit complicated, but it's not too bad.  Just remember to
use the \texttt{-H} option to specify the machines to run on, and always
use \texttt{-n 1}.  The rest is just the \texttt{R} command line to
...
\texttt{mpi.universe.size}.  That allows you to control the number of
workers that are spawned via the \texttt{orterun} command.
